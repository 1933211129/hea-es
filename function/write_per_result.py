"""
从 performance_prompt_text 表读取 prompt，调用大模型获取结果，写入 result 字段
"""
import psycopg2
import json
import re
...
def process_performance_results():
    ...
    try:
        ...
        # 检查 performance_prompt_text 表是否有 result 字段
        cursor.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_schema = 'public' 
            AND table_name = 'performance_prompt_text' 
        ...
        if not has_column:
            print("创建 result 字段...")
            cursor.execute("""
                ALTER TABLE performance_prompt_text 
        ...
        # 查询 result 为空的记录
        print("查询需要处理的记录（result 为空）...")
        cursor.execute("""
            SELECT id, performance_prompt_text 
            FROM performance_prompt_text 
            WHERE performance_prompt_text IS NOT NULL 
            AND performance_prompt_text != ''
            AND (result IS NULL OR result::text = 'null'::text)
        """)
        ...
        if len(records) == 0:
            print("没有找到需要处理的记录（所有记录的 result 都已填充）")
            return
        ...
        with tqdm(total=len(records), desc="处理大模型请求") as pbar:
            for idx, (record_id, performance_prompt_text) in enumerate(records):
                try:
                    # 每处理10条记录检查一次连接
                    ...
                    # 调用大模型
                    print(f"\n处理 ID: {record_id}")
                    result_json = get_llm_result_with_retry(performance_prompt_text)
                    
                    # 立即写入数据库
                    if result_json:
                        cursor.execute("""
                            UPDATE performance_prompt_text 
                            SET result = %s::jsonb
                            WHERE id = %s
